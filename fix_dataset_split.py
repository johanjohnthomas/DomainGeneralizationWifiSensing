#!/usr/bin/env python
"""
Script to fix dataset splitting issues by modifying the train/val/test split approach.
This script adapts the splitting strategy when there are insufficient groups for GroupShuffleSplit.
"""
...
subdir = "AR3a_R"  # Based on your error output
...
def fix_dataset_split(exp_dir, label):
    ...
    if use_regular_split:
        ...
        # This would be a copy of the original code
        print("Using GroupShuffleSplit approach (not implemented in this fix)")
        return
...
if __name__ == "__main__":
    print("=== Dataset Split Fix Tool ===")
    print(f"Fixing data split for subdirectory: {subdir}")
    
    # Execute the fix
    exp_dir, label = load_raw_data()
    if exp_dir and label:
        fix_dataset_split(exp_dir, label)
    else:
        print("Failed to load raw data. Fix aborted.")
    
    print("\nProcess complete. Check the output above for results.") 